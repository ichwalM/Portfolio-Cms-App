...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('dashboard.blogs.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-slate-900/50 backdrop-blur-md border border-white/5 rounded-2xl p-6">
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $post->title) }}" class="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors" placeholder="Enter post title...">
                        </div>
                        
                        <div>
                            <label for="slug" class="block text-sm font-medium text-slate-400 mb-2">Slug</label>
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $post->slug) }}" class="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors" placeholder="post-slug">
                        </div>
            ...
            <div class="space-y-6">
                <div class="bg-slate-900/50 backdrop-blur-md border border-white/5 rounded-2xl p-6">
                    ...
                    @if($post->thumbnail)
                        <div class="mb-4">
                            <img src="{{ asset('storage/' . $post->thumbnail) }}" alt="Thumbnail" class="w-full rounded-lg border border-white/10">
                        </div>
                    @endif
                    <div class="w-full border-2 border-dashed border-white/10 rounded-xl p-8 text-center transition-colors hover:border-indigo-500/50 cursor-pointer relative bg-slate-950/50">
                        ...
                        <p class="mt-1 text-sm text-slate-400">Change cover image</p>
                ...
                <div class="bg-slate-900/50 backdrop-blur-md border border-white/5 rounded-2xl p-6">
                    ...
                    <a href="{{ route('dashboard.blogs.index') }}" class="block w-full text-center mt-3 text-sm text-slate-500 hover:text-white transition-colors">Cancel</a>
                </div>
            </div>
        </div>
    </form>
