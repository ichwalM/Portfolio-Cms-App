...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('dashboard.wall-apps.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
                ...
                <div class="bg-slate-900/50 backdrop-blur-md border border-white/5 rounded-2xl p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors" placeholder="e.g. My Awesome App" required>
                        </div>
                        
                        <div>
                            <label for="url" class="block text-sm font-medium text-slate-400 mb-2">URL</label>
                            <input type="url" name="url" id="url" value="{{ old('url') }}" class="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors" placeholder="https://example.com" required>
                        </div>

                        <div>
                            <label for="description" class="block text-sm font-medium text-slate-400 mb-2">Description (Optional)</label>
                            <textarea name="description" id="description" rows="5" class="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors" placeholder="Describe the app...">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="bg-slate-900/50 backdrop-blur-md border border-white/5 rounded-2xl p-6">
                    ...
                    <div class="w-full border-2 border-dashed border-white/10 rounded-xl p-8 text-center transition-colors hover:border-indigo-500/50 cursor-pointer relative bg-slate-950/50">
                        <input type="file" name="icon" id="icon" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" required>
                        ...
                        <p class="mt-1 text-sm text-slate-400">Upload an icon</p>
